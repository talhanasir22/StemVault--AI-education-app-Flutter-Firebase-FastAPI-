...
import 'package:stem_vault/features/home/assign_uname_page.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final user = await _auth.studentSignIn(
          _emailController.text.trim(),
          _passwordController.text.trim(),
        );

        if (user != null) {
          var userDoc = await firestoreServices.getStudentData(user.uid);
          ...
          if (userDoc != null && userDoc.exists) {
            ...
            if (userData != null && userData.containsKey('userName') && userData['userName'].isNotEmpty) {
              ...
              Navigator.pushReplacement(context, PageTransition(type: PageTransitionType.rightToLeft, child: AssignUserNamePage()));
            }
          } else {
                              ...
                              if (user != null) {
                                ...
                                if (userDoc!.exists) {
                                  ...
                                  if (userData != null && userData['userName'] != null && userData['userName'].toString().isNotEmpty) {
                                    // If username exists, navigate to BottomNavBar
                                    Navigator.pushReplacement(
                                      context,
                                      PageTransition(type: PageTransitionType.rightToLeft, child: BottomNavBar()),
                                    );
                                  } else {
                                    // If username is missing, navigate to AssignUserNamePage
                                    Navigator.pushReplacement(
                                      context,
                                      PageTransition(type: PageTransitionType.rightToLeft, child: AssignUserNamePage()),
                                    );
                                  }
                                } else {
                                  ...
                                  await firestoreServices.saveStudentData(newUser);

                                  // Navigate to AssignUserNamePage
                                  Navigator.pushReplacement(
                                    context,
                                    PageTransition(type: PageTransitionType.rightToLeft, child: AssignUserNamePage()),
                                  );
